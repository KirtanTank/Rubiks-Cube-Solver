...
def proximity(pos, target):
    delta = 0.3
    result = False
    if pos.x + delta > target[0] > pos.x - delta:
        if pos.y + delta > target[1] > pos.y - delta:
            if pos.z + delta > target[2] > pos.z - delta:
                result = True
    return result
...
def solve(cube):
    ...
    print("values in solve fun in solve_rubiccs_cube.py: ", values)
